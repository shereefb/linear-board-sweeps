...
- **Dev sweep** - "Ready for Dev" -> "In Review" (writes code on a worktree, claims active work with `dev:in-progress`, gated security/perf/design lenses, code review + an independent reviewer subagent, verifies green, pushes the branch, no merge): `.claude/skills/dev-sweep/SKILL.md`.
