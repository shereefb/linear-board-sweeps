...
- **Ship sweep** - "Ready to Ship" -> "Done" (the only sweep that merges + deploys; canary-verifies; resume-on-merge-commit): `.claude/skills/ship-sweep/SKILL.md`. **Single-runner** (pin dispatch to one host) and fires only on cards a human moved into "Ready to Ship". This repo also requires the `ship:approved` label.
