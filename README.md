...
- **Machine-independent.** All work and tooling flow through origin; launchd runs from a managed clean kit clone so day-to-day edits in this checkout do not block scheduled dispatch, and skills auto-update by fast-forwarding that managed kit clone and pushing refreshed skills to each anchor. Planned COD-116 hardening extends the same idea to managed workspace clones so dirty human anchor checkouts become advisory instead of scheduled dispatch blockers.
...
- `COD-116`: planned managed workspace clones, exact dirty-path diagnostics, artifact isolation, independent dirty-checkout recovery, and a `doctor` command so scheduled sweeps are no longer coupled to dirty human checkouts.
