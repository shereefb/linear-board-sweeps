...
- **Claude usage fallback.** Each scheduled Codex stage may declare one optional Claude `fallback` beside its primary runtime. The launcher uses it only after a source-backed, positively classified Codex personal/model usage-exhaustion event; workspace credits/spend caps and ordinary agent, auth, model, network, overload, signal, and transient rate-limit failures fail closed. Both attempts share one card claim, worktree, and one capacity reservation with at most two sequential attempts before final reconciliation. Run records preserve a normalized run-record `attempts` array for the primary attempt and any Claude fallback attempt.
