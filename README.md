...
- **Bounded queue draining.** `parallel.maxDrainPasses` defaults to `2` and is clamped to `1..5`. After a dispatch finishes, the launcher re-scans queues before exiting, so cards added during the run can be picked up without waiting for the next timer tick.
