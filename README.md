...
**Shipped (COD-159):** security-sensitive work declares `Trust-boundary contract: trust-boundary-contract/v1 — required|not required — <non-empty rationale>` in both fixed-snapshot spec and plan artifacts. Required contracts use contiguous, unique `TB1`…`TBn` IDs with matching sets; the helper classifies their Git ancestry as `legacy`, `current`, or fail-closed `incomparable`. Only two legacy artifacts may omit the declaration; current, mixed, malformed, or incomparable artifacts bounce directly from Dev or QA to Spec. Sink-specific prose remains a documented review policy with residual risk, not a universal sanitizer. Existing Factory Learning security-review events and the human Ship gate are unchanged; verification uses inert local fixtures only, with no production attack testing.
...
- **Immediate non-production handoffs.** After a successful spec handoff, the parent launcher can immediately dispatch dev for that same card; after successful dev, it can immediately dispatch QA. `parallel.maxHandoffTriggerHops` defaults to `2`, clamps to `0..3`, and `0` disables this. A parent tick spends at most `parallel.maxNonShipDispatches` handoff dispatch slots, while completion refill has its own `parallel.maxSameRepoRefillDispatches` budget. QA still stops at the human signoff queue; Ship is never handoff-triggered, and Ship refill only considers cards a human already moved into `Ship`.
