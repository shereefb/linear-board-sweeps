...
- **Trusted dispatch failure evidence (COD-288).** Factory Learning derives repeated dispatch failures only from the parent launcher's classified start, I/O, exit, and signal outcomes, never child `terminal/failed` audit events. Routing deferrals and interruptions remain operationally visible but are excluded from this learning signal. Cause-specific v2 evidence keeps legacy COD-288 measurement separate, and a zero-failure window requires trusted successful-delivery exposure from every declared contributor before it can verify improvement.
