...
- **Runtime executable preflight.** After choosing the configured runtime, executable resolution is the matching `CODEX_BIN` or `CLAUDE_BIN` environment override, then `PATH`, then the ChatGPT.app bundled Codex, then the legacy Codex.app bundle, then fail before claim. A missing runtime therefore cannot leave a new workflow claim behind.
