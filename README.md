...
- **Bounded non-ship parallelism.** `parallel.maxNonShipDispatches` defaults to `2`, so one tick can dispatch a small batch of non-ship workspace/stage candidates. Distinct Spec, Dev, and QA candidates from one registered workspace may run together; resolved repository overlap remains exclusive across different registered workspaces. Set the limit to `1` for serial mode. ship always stays serial.
- **Same-repo card slots.** Inside a selected non-ship workspace/stage candidate, `parallel.sameRepoCardLimits` defaults to `spec:4`, `dev:4`, `qa:1`, `ship:1`. The parent launcher claims exact cards with owner-token heartbeats, dispatches child agents with `AUTO_SWEEP_ISSUE` and isolated worktree/log/temp/port paths, and writes per-card run records. `maxNonShipDispatches` counts workspace/stage candidates; `sameRepoCardLimits` counts active child card slots per primary repo and stage.
