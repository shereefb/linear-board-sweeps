...
- **Self-healing.** A crashed session's exact declaration is closed before its claim label is released; heartbeat age is only the liveness clock. When a child exits successfully but leaves its card in the same workflow state, the launcher immediately closes and releases only that immutable epoch. Observed nonzero exits, OS signals, and typed log-I/O terminations use declaration-bound retry evidence; a direct log-I/O termination takes precedence over partial capacity text or child deferral files, while ordinary capacity recovery remains unchanged. Genuinely silent children remain on the crash reaper path. Poison/oscillating cards escalate to `blocked:needs-user`, manual/dedicated work can be parked with `sweep:manual-only`, a holding-state reaper releases claims stranded in `Signoff`, scheduled tick failures create self-clearing `Todo` cards when Linear is reachable, and a PID-liveness lock keeps exactly one launcher tick supervising a bounded child-agent batch at a time.
