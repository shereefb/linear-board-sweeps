...
| `scripts/linear.mjs` | Zero-dependency Linear engine (Node 18+): `whoami`, `setup-team`, `ensure-project`, `create-card`, `move-card-bottom`, `move-card-bottom-if-current`, `retire-state`, `rename-states`, `repo-status`, `dependency-status`, `query`. |
| `scripts/linear-watch.mjs` | Zero-dependency auto-sweep launcher: workspace registration/activation, runner pins, `claim-migration-status`, `claim-migration-reset`, `learning-status`, `learning-run [--dry-run]`, `tick`, `health`, and `doctor`. Polls Linear cheaply and dispatches only actionable work — see [Triggering](#triggering-auto-sweep). |
...
- **Claims are immutable epochs.** The launcher exports separate `AUTO_SWEEP_OWNER_TOKEN` and `AUTO_SWEEP_CLAIM_DECLARATION` values. Ownership is first-declaration-wins over complete Linear comment history; declaration-scoped heartbeats are liveness only. Every release, reaping action, and guarded terminal mutation posts and verifies the exact `[auto-sweep-claim-close v1 ...]` before changing the label or state. Before enabling the new launcher everywhere, run `node scripts/linear-watch.mjs claim-migration-status --json`; it complete-reads every project card, including unlabeled old declarations. An attended `claim-migration-reset <anchor> <Issue> <Claim> <Target>` accepts only `legacy-unowned` with target `legacy`, or `orphan-declaration` with the exact declaration ID. For `ambiguous`, inspect and correct/remove the exact malformed or conflicting Linear marker, then rerun `claim-migration-status`; reset never repairs ambiguity. Rollback must not restore a heartbeat-owner writer after declarations exist.
- **Self-healing.** A crashed session's exact declaration is closed before its claim label is released; heartbeat age is only the liveness clock. When a child exits successfully but leaves its card in the same workflow state, the launcher immediately closes and releases only that immutable epoch. Every present child outcome file is a closed protocol boundary: an invalid file fails the child protocol instead of being treated as absent. Each child starts only after inherited card-scoped `AUTO_SWEEP_*` identity is scrubbed and the launcher’s current identity overlay is applied. Observed failures use declaration-bound retry evidence; genuinely silent children remain on the crash reaper path. Poison/oscillating cards escalate to `blocked:needs-user`, manual/dedicated work can be parked with `sweep:manual-only`, a holding-state reaper releases claims stranded in `Signoff`, scheduled tick failures create self-clearing `Todo` cards when Linear is reachable, and a route Todo closes only after a healthy tick proves the exact source workspace, card, and stage target recovered. A PID-liveness lock keeps exactly one launcher tick supervising a bounded child-agent batch at a time.
...
- **Same-repo card slots.** Inside a selected non-ship workspace/stage candidate, `parallel.sameRepoCardLimits` defaults to `spec:4`, `dev:4`, `qa:1`, `ship:1`. The parent launcher claims exact cards with immutable declarations and declaration-scoped liveness heartbeats, dispatches child agents with `AUTO_SWEEP_ISSUE` and isolated worktree/log/temp/port paths, and writes per-card run records. `maxNonShipDispatches` counts non-Ship workspace/stage candidates; `sameRepoCardLimits` counts active child card slots per primary repo and stage. Ship is always one card per selected source workspace.
...
- **Quiet hourly usage recovery.** Each scheduled Codex stage may declare one optional Claude `fallback`. Confirmed Codex exhaustion immediately continues the same card on Claude and cools Codex host-wide for one hour, so other cards skip sacrificial Codex attempts. Both processes share one card claim, worktree, and one capacity reservation with at most two sequential attempts; the normalized run-record `attempts` array records which lanes ran. Confirmed Claude exhaustion cools Claude too. If every configured runtime is cooling, the launcher silently preserves exact claims/worktrees and admits one probe after an hour—no Linear Todo, comment, label, or state churn. Success clears the cooldown; repeated exhaustion renews it. Ordinary auth, model, network, overload, signal, and transient rate-limit failures still fail closed; missing executables, invalid configuration, agent failures, and unrecognized output remain actionable. `doctor` exposes cooldowns and next probes locally.
...
- `COD-291`: shipped launcher-authoritative scheduled routing, no-I/O non-routed `repo-status` success, closed child-outcome validation, child identity scrubbing, and exact workspace/card/stage route-Todo recovery with fail-closed legacy aliasing.
