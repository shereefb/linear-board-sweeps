...
- **Trusted dispatch failure evidence (planned, COD-288).** Factory Learning will derive repeated dispatch failures only from the parent launcher's classified start, I/O, exit, and signal outcomes, not child `terminal/failed` audit events. Cause-specific v2 evidence will keep legacy COD-288 measurement separate and require trusted contributor exposure before a zero-failure window can verify improvement.
...
- `COD-295`: Spec validates each final exact Spec/plan pair through the launcher-source helper before commit or a Dev move, remaining in Spec on failure. Dev keeps its independent validator and honest `missing-design` evidence; the capability/restart boundary and issue-bound terminal outcome keep scheduled handoff failures fail-closed without relying on agent exit status.
