...
For rollout, `node scripts/linear-watch.mjs claim-migration-status --json` performs a read-only registered-workspace scan with complete claim histories and bounded identifiers/reasons. A nonzero result means legacy labels, orphan declarations, ambiguous histories, or unreadable workspaces must drain or be reset through the launcher's verified attended cleanup before rollout. Never manufacture a declaration for a legacy label. Rollback may pause launchers, but must not reintroduce heartbeat-owner authority after declaration epochs exist; close or reset them first.
