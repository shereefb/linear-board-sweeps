...
An in-progress claim is an immutable epoch. The first well-formed `[auto-sweep-claim v1 claim=<claim> owner=<owner> declaration=<declaration>]` after the prior close/reset wins, based on complete Linear comment history. `AUTO_SWEEP_OWNER_TOKEN` and `AUTO_SWEEP_CLAIM_DECLARATION` remain separate. Declaration heartbeats prove liveness only and never transfer ownership. Before a successful exit, reaping action, label removal, or terminal state mutation, the actor re-reads complete history, proves both exact tokens, posts and verifies `[auto-sweep-claim-close v1 ...]`, then mutates. A freed Ship slot may refill from any correctly routed Ship card in the same registered source workspace, while the capacity ledger continues to allow only one active Ship child for that workspace.

For rollout, `node scripts/linear-watch.mjs claim-migration-status --json` performs a read-only registered-workspace scan that complete-reads every project card, with bounded identifiers/reasons and complete counts. An attended `claim-migration-reset <anchor> <Issue> <Claim> <Target>` accepts only `legacy-unowned` with target `legacy`, or `orphan-declaration` with the exact declaration ID. For `ambiguous`, inspect and correct/remove the exact malformed or conflicting Linear marker, then rerun `claim-migration-status`; reset never repairs ambiguity and rollout remains blocked. Never manufacture a declaration for a legacy label. Rollback may pause launchers, but must not reintroduce heartbeat-owner authority after declaration epochs exist.

After an observed nonzero child exit, OS signal, or typed `dispatch-io-error` caused by the launcher log sink, the launcher writes declaration-bound retry evidence, complete-reads and verifies the exact immutable epoch, closes it, and only then removes its claim label. A direct log-I/O termination cannot be reclassified from partial capacity text or replaced by a child dependency/repository deferral file; ordinary non-I/O capacity and deferral recovery retain their existing precedence. The marker cools only that claim stage for its existing 45/90/120-minute stale interval. Failed results do not trigger same-tick refill or handoff. Silent/frozen children continue through `[auto-sweep-reaper]` and declaration-scoped stale-claim evidence.
...
The launcher and sweeps write/read a few **audit-marker comments** on cards (you don't create these by hand): `[auto-sweep-claim v1 ...]` (immutable ownership), `[auto-sweep-heartbeat v1 claim=<claim> declaration=<declaration> at=<ISO>]` (liveness only), `[auto-sweep-claim-close v1 ...]` (authority removed before mutation), `[auto-sweep-reaper]` (a stale claim it auto-released), `[auto-sweep-orphan]` (a launcher-owned claim released after a start/defer failure or a successful same-state child exit), declaration-bound retry evidence, `[auto-sweep-bounce <from>→<to>]`, `[auto-sweep-fast-path <KEY> head=<full-git-sha>]`, `[auto-sweep-auto-ship <KEY> head=<full-git-sha>]`, `[auto-sweep-tick-failure <fingerprint>]`, and `[auto-sweep-tick-recovered <fingerprint> <ISO>]`.
