...
Verification evidence (2026-07-11): all four syntax checks passed; after merging current `main` and resolving the COD-144/COD-148/COD-175 interactions, the focused launcher suite passed 399/399 and the full suite passed 642/642. All six canonical skill mirrors matched byte-for-byte. Delayed-claim races were reproduced RED and fixed GREEN for acquisition cleanup, own reset/reaping, terminal close verification, provider/capacity resume retention, client/server clock skew, and repeated resume-resolution notices. A direct Ship-refill regression covers demand through `claimCardSlots` into paired child environment identity. Independent merge re-review approved the final fixes with no remaining correctness finding; the ship coverage audit assessed changed-path coverage at 94%. The live migration diagnostic remained intentionally non-ready with five legacy-unowned labels and no orphan or ambiguous histories; rollout stays blocked until attended migration cleanup.

- [x] **Step 6: Update COD-169**
