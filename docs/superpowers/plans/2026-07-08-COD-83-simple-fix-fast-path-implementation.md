...
Add an opt-in policy that lets dev-sweep mark tiny high-confidence changes as eligible for a human to move directly to Ready to Ship, without merging or deploying.
...
1. Add `fastPath` defaults to the template config, disabled by default.
...
6. Update ship-sweep sanity: accept `qa:passed`, or accept `fast-path:eligible` only when the card is already in Ready to Ship by human action; still honor `requireShipApproval`.
...
Land disabled. Enable only after an attended run on one low-risk repo.
