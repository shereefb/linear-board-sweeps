...
3. **Worktree = reconstruct-and-reconcile, never assume-clean.** Worktrees are the one genuinely machine-local git artifact — disposable caches; the branch is the truth. On picking up an actively claimed `Ready for Dev` card or an `In Review` card, the skill runs `git fetch` in each candidate repo, then:
   ...
   - **Local worktree exists and is dirty** → preserve it exactly. Never reset, checkout, restore, clean, stash, auto-commit, or broadly remove it. A successful child that remains in its stage may release its claim only after proving the worktree clean and its branch visible on origin; otherwise the launcher retains the exact claim/worktree tuple in a local resume record and resumes it on a later eligible tick.
