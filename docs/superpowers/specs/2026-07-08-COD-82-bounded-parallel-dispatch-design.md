...
Ship remains outside the pool. If any ship candidate exists and this host is the ship runner, the tick dispatches exactly one ship pass and dispatches no other work. That ship pass drains the Ready to Ship queue serially, one card at a time, with a final empty-queue recheck before it exits. This preserves the "one production deploy at a time" invariant and keeps the existing `shipRunner` protection meaningful without making approved cards wait for later ticks.
