...
| Ship | Codex | `gpt-5.6-terra` | `medium` |

The independent reviewer role remains `claude-opus-4-8`.

## Verified Model Identifiers

The locally authenticated Codex catalog from `codex debug models` lists the exact Sol and Terra model slugs. It describes Sol as the latest frontier agentic coding model and Terra as the balanced model for everyday agentic coding. The catalog supports the requested `high` and `medium` effort levels for both models.
...
The `safetaper-coach` anchor is detached because the clean `.worktrees/SAF-205` checkout owns local `main`. Apply and commit the same config change in that main worktree, push it normally, then move the detached anchor checkout to the resulting main commit so both local paths are clean and synchronized. Before every commit, fetch and confirm that the target branch is still aligned with its remote; do not overwrite concurrent work.
...
- Ship remains serial, human-gated, canary-verified, and fail-closed; Terra medium adds balanced operational reasoning without changing those workflow gates.
