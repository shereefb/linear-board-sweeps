...
- **Claim rollout gate** = run `node scripts/linear-watch.mjs claim-migration-status --json` on the launcher host. It read-only scans every registered workspace, complete-hydrates every project card's claim history, and exits nonzero for legacy labels, orphan declarations, ambiguous history, unreadable workspaces, or other non-rollout-safe epochs. An attended `claim-migration-reset <anchor> <Issue> <Claim> <Target>` accepts only `legacy-unowned` with target `legacy`, or `orphan-declaration` with the exact declaration ID. For `ambiguous`, inspect and correct/remove the exact malformed or conflicting Linear marker, then rerun `claim-migration-status`; reset never repairs ambiguity. Ownership remains first-declaration-wins, heartbeats remain liveness only, and rollback must pause launchers and close/reset valid declarations before installing heartbeat-owner code.
