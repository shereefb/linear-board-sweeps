...
1. **Ensure runtime selection exists** in `ANCHOR/.claude/linear-sweep.json`. If it was copied from the template (Step 7) it already has this. If not, add:
   ...
   ```
   The launcher resolves `runtimes.<sweep>` first, then legacy `runtime` + `models.<sweep>`, then Codex defaults. The default scheduled sweeps use Codex so unattended launchd ticks do not depend on a separate Claude login. Each scheduled Codex entry above has one singular optional Claude `fallback`; it runs only after a source-backed, positively classified Codex personal/model usage-exhaustion event. Workspace credits/spend caps and every generic failure stay fail closed. Remove `fallback` from a stage to disable its fallback. Use explicit supported best-model overrides so scheduled sweeps do not silently drift with runtime defaults. `runtimes.review` is a reviewer role preference for the sweep instructions, not a scheduled stage.
