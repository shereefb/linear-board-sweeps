...
**Claim ownership:** the launcher exports each claimed child's owner as `AUTO_SWEEP_OWNER_TOKEN`. Claim heartbeats use `[auto-sweep-heartbeat <ISO> owner=<token> claim=<claim>]`; a terminal helper may remove a claim only when the latest complete Linear comment read proves that exact token still owns that exact state-mapped claim.
