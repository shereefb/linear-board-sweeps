...
**Trust-boundary contract:** security-sensitive Spec, Dev, and QA work uses `trust-boundary-contract/v1` with exact `TB1`…`TBn` IDs. The trusted rollout marker is exactly `1.2.0.6\n`; classify fixed target blobs as `legacy`, `current`, or fail-closed `incomparable`. Only two legacy artifacts may omit a declaration; every current, mixed, malformed, or incomparable result bounces Dev/QA directly to Spec. Scheduled runs use the trusted kit helper; manual runs materialize and hash-verify the regular helper blob from rollout `R`, never a worktree or later-main helper. Treat sink prose as a review policy with residual risk, preserve Factory Learning events and the human Ship gate, and use inert local fixtures rather than production attack testing.
...
- **Ship sweep** — "Ship" → "Done" (the only sweep that MERGES + DEPLOYS TO PROD; canary-verifies; resume-on-merge-commit): `.claude/skills/ship-sweep/SKILL.md`. **Single-runner** (pin dispatch to one host) and fires only on cards a human moved into "Ship".
