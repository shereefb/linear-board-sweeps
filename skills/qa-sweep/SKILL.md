...
Scheduled single-card runs require both `AUTO_SWEEP_OWNER_TOKEN` and `AUTO_SWEEP_CLAIM_DECLARATION`; if either is absent, stop before material work. The launcher has already posted `[auto-sweep-claim v1 claim=qa:in-progress owner=<owner> declaration=<declaration>]` before adding the label. An attended run creates separate random owner and declaration tokens, posts that declaration before adding `qa:in-progress`, then complete-reads all claim comments and continues only if the exact declaration is the first-declaration-wins owner of the labeled epoch.

While holding the claim, post `[auto-sweep-heartbeat v1 claim=qa:in-progress declaration=<declaration> at=<ISO8601>]`. Heartbeats are liveness only; never ownership. Before every claim-affecting exit or mutation, complete-read claim history and verify the exact owner plus declaration, post `[auto-sweep-claim-close v1 claim=qa:in-progress declaration=<declaration> reason=<released|reaped|orphaned|terminal|blocked|failed>]`, verify that exact close, and only then remove the label or perform the guarded terminal move. A close removes authority immediately even if the later mutation fails.

## 1. Select cards (top-of-column order, bounded, claimed)

**Single-card auto-sweep mode.** If `AUTO_SWEEP_ISSUE` is set (or the unattended prompt names a single issue key), process only that issue and ignore every other QA card. A scheduled child receives the launcher's nonempty owner and declaration variables; treat that exact first-declaration-wins epoch as this child's pre-claim, not as a competing run. Missing/mismatched declaration ownership fails closed. Use `AUTO_SWEEP_WORKTREE`, `AUTO_SWEEP_LOG_DIR`, `AUTO_SWEEP_TMPDIR`, `AUTO_SWEEP_APP_PORT`, `AUTO_SWEEP_SCREENSHOT_DIR`, and `AUTO_SWEEP_BROWSER_PROFILE_DIR` when present instead of inventing local paths, ports, screenshot directories, or browser profiles. Store screenshots, generated evidence, browser profiles, and scratch files under those env paths, never in repo roots.
...
- **Claim** with `qa:in-progress` before starting. A manual QA run creates separate random owner and declaration tokens and follows the immutable claim protocol above. Remove the claim only through an exact declaration-proven close path when you finish, block, or bail.
...
3. **Exercise the feature as a user, in detail — use `/qa` as the test engine** (or `/qa-only` for the report form) rather than ad-hoc clicking. Walk the actual user flows the card adds — happy path AND the obvious edge cases. Click through, fill forms, trigger the states. Watch console + network + server logs for errors. Use subagents/parallel work for independent flows. If a `/plan-eng-review` test plan exists for the branch, feed it in as primary test input. For `verification-contract/v1 — required`, use the plan's `V1..Vn` IDs beside the engineering test plan and record a disposition for every `V1..Vn` ID: exercised product evidence for user-observable behavior; for behavior that cannot be induced safely, the exact green lower-level command/assertion plus nearest public outcome; or an approved exception with owner and residual risk. One covered ID never hides an uncovered sibling. Any missing disposition blocks the gate: do not add `qa:passed`. Any newly verified material missing obligation remains a `review/test-gap` finding and returns through the normal changes/design path.
...
7. **Eligible after the final rerun:** post `[auto-sweep-auto-ship <KEY> head=<full-git-sha>]` with the matching reviewed/current-origin SHA evidence and policy facts, then run `node scripts/linear.mjs move-card-bottom-if-current <PREFIX-###> "QA" "Ship" "qa:in-progress" "$AUTO_SWEEP_OWNER_TOKEN" "$AUTO_SWEEP_CLAIM_DECLARATION"`. This is a queue transition only: do not invoke ship-sweep directly and do not add a launcher handoff.
8. **SHA mismatch:** remove `fast-path:eligible` because it is stale, record the reviewed SHA and current origin SHA in the review write-up, and run `node scripts/linear.mjs move-card-bottom-if-current <PREFIX-###> "QA" "Signoff" "qa:in-progress" "$AUTO_SWEEP_OWNER_TOKEN" "$AUTO_SWEEP_CLAIM_DECLARATION"`.
9. **Every other denial:** record the policy reason and move the passing card to the bottom of `Signoff` with the same guarded helper. A policy denial is not a QA failure: keep `qa:passed` and do not add `qa:needs-changes`.

Do not separately release `qa:in-progress` on either passing terminal path. `move-card-bottom-if-current` obtains destination metadata/pagination and bottom rank first, then makes its final complete state/labels/comments guard read immediately before mutation. It requires the exact owner and declaration to own the first-declaration-wins epoch, closes and verifies that declaration before mutation, and denies live state/claim/blocker/foreign-claim/ownership changes (exit `3`; incomplete/unreadable facts are exit `2`). On success, one `issueUpdate` mutation contains destination state, bottom rank, and `removedLabelIds` with exactly the proven `qa:in-progress` label ID; unrelated or concurrently added labels are never replaced. Linear exposes no compare-and-swap for this transition; this is the narrowest supported transition, not an atomic CAS. If ownership changed, stop and never remove the newer owner's claim. **Leave the branch pushed and unmerged; do not delete the worktree/branch/PR** — ship-sweep needs them. Normal cards continue through `QA` → `Signoff` → human approval → `Ship`; only an eligible unchanged fast path takes automatic `QA` → `Ship`, and `requireShipApproval: true` always preserves `Signoff`.
...
- **Heartbeat while you hold a claim.** Roughly every 5 minutes, post the declaration heartbeat from the immutable claim protocol. It is liveness only; never ownership. If the complete-history resolver no longer returns the exact owner and declaration, stop and never remove the claim. A claim with no heartbeat past its stale threshold is treated as crashed and auto-released by the launcher — QA runs are long, so heartbeat diligently.
