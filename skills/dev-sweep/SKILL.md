...
- **Spec-quality gate:** expect excellent specs. If a card is under-specified (no clear spec/plan, ambiguous acceptance, missing design decisions), **move it to the bottom of "Spec"** with a comment naming exactly what's under-specified, and leave it — do NOT develop from a weak spec. Prefer the repo helper (`node scripts/linear.mjs move-card-bottom <PREFIX-###> "Spec"`) so the status and bottom rank update together. (The spec-sweep loop re-specs it.) **Verification-contract gate.** Run `node scripts/verification-contract.mjs validate --spec <spec-path> --plan <plan-path>` before coding. A valid `required` pair has unique `V1..Vn` IDs, exactly one V source for every required correctness C ID, and complete plan mappings; `not required` is accepted only when the actual plan/diff has no material verification surface. The helper alone classifies absent declarations by comparing the artifact first-add commit, the `.sweep-version` rollout boundary, and the first commit that introduced the versioned contract literal into the installed/canonical Spec skill: proven older artifacts use the legacy quality gate; post-boundary, missing, or shallow history must fail closed; incomparable history must fail closed and bounces `missing-design` with the exact bounded diagnostic. Never reimplement rollout ancestry in skill prose.
