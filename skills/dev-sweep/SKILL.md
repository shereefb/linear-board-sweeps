...
- **Correctness-contract gate.** Read the spec/plan audit for `Correctness contract: correctness-contract/v1`. `required` needs both the invariant and traceability tables; missing material is a `missing-design` bounce. `not required` is accepted at this pre-build gate only when the approved plan has no material correctness surface. If the declaration is absent, pin the rollout boundary to the first anchor commit whose `.claude/skills/.sweep-version` content is exactly `1.2.0.6`, regardless of later kit upgrades, and compare it with the commit that first introduced the artifact by running `node "$AUTO_SWEEP_KIT_PATH/scripts/correctness-boundary.mjs" <anchor> <artifact-path>`. Only a proven `legacy` result may use the legacy quality gate. Missing or incomparable history must fail closed; `post-rollout` also bounces as `missing-design`, always with the command's exact evidence gap.
...
5. **Execute declared correctness proofs.** For `correctness-contract/v1 — required`, map every `C1..Cn` ID to the actual diff and its already-declared test/assertion, then execute those narrow proofs before the full suite. Fix implementation failures. This is proof execution, not a third review pass. If mapping/execution instead exposes a material contract omission or defect, emit the existing `review/correctness` learning event and bounce to Spec as `missing-design`; never relabel discovery as ordinary implementation work to improve the metric. Before code review, re-check the actual diff for a `not required` declaration; if it exposes a material correctness surface, bounce to Spec as `missing-design`. Include the invariant-to-diff/test proof map in the QA handoff.
