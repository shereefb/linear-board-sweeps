...
5. Evaluate `qaHandoffDecision` with explicit named input mappings: `fastPathEnabled: config.fastPath.enabled !== false`, `requireShipApproval: config.requireShipApproval`, state `QA`, current labels including `fast-path:eligible` and `qa:passed`, the issue identifier, both full SHAs, and any live foreign `*:in-progress` claim. Pass the raw approval value; eligibility requires that raw value to be `false`. Unknown, unreadable, or malformed evidence fails closed to `Signoff`.
