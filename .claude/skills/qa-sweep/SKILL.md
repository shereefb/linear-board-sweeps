...
7. **Eligible after the final rerun:** post `[auto-sweep-auto-ship <KEY> head=<full-git-sha>]` with the matching reviewed/current-origin SHA evidence and policy facts, then run `node scripts/linear.mjs move-card-bottom-if-current <PREFIX-###> "QA" "Ship" "qa:in-progress"`. This is a queue transition only: do not invoke ship-sweep directly and do not add a launcher handoff.
8. **SHA mismatch:** remove `fast-path:eligible` because it is stale, record the reviewed SHA and current origin SHA in the review write-up, and run `node scripts/linear.mjs move-card-bottom-if-current <PREFIX-###> "QA" "Signoff" "qa:in-progress"`.
...
Do not separately release `qa:in-progress` on either passing terminal path. `move-card-bottom-if-current` fresh-reads the exact source state and all label IDs, denies live state/claim/blocker/foreign-claim changes (exit `3`; unreadable is exit `2`), and on success performs one `issueUpdate` mutation containing the destination state, bottom rank, and label IDs with only `qa:in-progress` removed. Linear exposes no compare-and-swap for this transition; this is the narrowest supported transition, not an atomic CAS. If the helper denies, stop without overriding live facts. **Leave the branch pushed and unmerged; do not delete the worktree/branch/PR** — ship-sweep needs them. Normal cards continue through `QA` → `Signoff` → human approval → `Ship`; only an eligible unchanged fast path takes automatic `QA` → `Ship`, and `requireShipApproval: true` always preserves `Signoff`.
