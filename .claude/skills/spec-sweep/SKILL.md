...
5. **Run every pre-plan review selected by the tier.** For a Tier 1 spec target or Tier 2, invoke `/plan-eng-review` on the spec and run the card's independent adversarial reviewer against that spec. Require the reviewer to trace every reuse/mechanism/premise claim against real code with file:line evidence, and fold verified findings into the spec before continuing. For `verification-contract/v1 — required`, challenge whether every material requirement, error path, regression surface, and correctness invariant has an obligation; each proof can fail for the named bad implementation; the chosen layer is the lowest stable boundary; broad suite commands do not replace named assertions; and every manual exception carries explicit residual risk. Emit every verified material omission through the existing `review/test-gap` event; contract compliance never suppresses or reclassifies a finding. A Tier 0 has no independent reviewer. A Tier 1 plan target defers both its plan pass and independent adversarial reviewer until after plan generation in step 8.
