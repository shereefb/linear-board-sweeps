...
8. **Reassess review depth from the completed plan.** Use its concrete file/module map, interfaces, dependency graph, task count and ordering, test plan, failure modes, rollout steps, and newly exposed risk surfaces. If a plan task exposes a new surface, add an `S` row, reconcile traceability, reassess tier monotonically, and run newly required reviews. A Tier 1 plan target now runs its required review after plan generation: invoke the plan pass, then run the independent adversarial reviewer against the plan and fold verified findings in. The final tier may stay the same or increase; it may never decrease. Run every newly required review before handoff:
