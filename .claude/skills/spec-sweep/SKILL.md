...
4. **Run every pre-plan review selected by the tier.** For a Tier 1 spec target or Tier 2, invoke `/plan-eng-review` on the spec and run the card's independent adversarial reviewer against that spec. Require the reviewer to trace every reuse/mechanism/premise claim against real code with file:line evidence, and fold verified findings into the spec before continuing. A Tier 0 has no independent reviewer. A Tier 1 plan target defers both its plan pass and independent adversarial reviewer until after plan generation in step 8.
5. **Run materially applicable specialized lenses.** Domain labels are candidates, not automatic proof that a heavyweight lens is useful. Do NOT run `/autoplan` (it bundles CEO review, which is deliberately out of this pipeline). Record every candidate lens as run or skipped with a materiality rationale:
