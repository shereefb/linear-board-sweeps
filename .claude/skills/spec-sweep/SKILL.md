...
4. **Run every pre-plan review selected by the tier.** For a Tier 1 spec target or Tier 2, invoke `/plan-eng-review` on the spec. Run one independent adversarial reviewer for every non-Tier-0 card: pair it with the Tier 1 target, or with the Tier 2 spec pass, and require it to trace every reuse/mechanism/premise claim against real code with file:line evidence. Fold verified findings into the spec before continuing. A Tier 0 or Tier 1 plan target has no pre-plan engineering pass.
5. **Run materially applicable specialized lenses.** Domain labels are candidates, not automatic proof that a heavyweight lens is useful. Do NOT run `/autoplan` (it bundles CEO review, which is deliberately out of this pipeline). Record every candidate lens as run or skipped with a materiality rationale:
   ...
   - **Performance:** actual performance-sensitive work runs the configured performance lens.
...
8. **Reassess review depth from the completed plan.** Use its concrete file/module map, interfaces, dependency graph, task count and ordering, test plan, failure modes, rollout steps, and newly exposed risk surfaces. The final tier may stay the same or increase; it may never decrease. Run every newly required review before handoff:
   - Tier 0 → Tier 1: run the plan pass.
   - Tier 0 → Tier 2: run the plan pass; also run the missed spec pass and regenerate/reconcile the plan if the growth exposes an unresolved design premise.
