...
8. **Write the implementation plan.** Include a **Repo scope** section naming the owning repo(s), plus a `Spec-sweep review audit` that records initial tier, predicted footprint, risk surfaces, selected/skipped reviews, and current outcomes. For each `performance-contract/v1 — required` row, add a `Performance traceability` table with `P` ID, `Proof kind`, `Implementing task(s)`, `Proof command / fixture`, `Expected evidence and pass condition`, `QA observation`, and `Residual risk`. Use one shared legacy-boundary decision for versioned contracts: compare when the artifact was first introduced with the installed `.sweep-version`. If performance is first (including COD-158 first), install or create the shared decision; if correctness is present (including COD-155 first), reuse or extend that shared decision. Proven pre-boundary artifacts use the legacy gate; missing or incomparable history must fail closed. If any required repo is outside `config.repos` or has no deploy path in `config.deploy`, stop and comment the split/config update needed instead of sending the card to Dev.
